...
// JWT token is read from localStorage key: 'auth_token'
// ============================================================
...
async function calculateManagerAnalytics() {
    const summary = await apiFetch(`${API.finance}/summary`);
    if (!summary) return;

    const el = id => document.getElementById(id);
    if (el('statRevenue'))   el('statRevenue').textContent   = formatCurrency(summary.total_revenue ?? 0);
    if (el('statOccupancy')) el('statOccupancy').textContent = `${parseFloat(summary.occupancy_rate ?? 0).toFixed(1)}%`;
    if (el('statInvoices'))  el('statInvoices').textContent  = `${summary.pending_invoices ?? 0} Pending`;
}
